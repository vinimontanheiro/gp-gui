...
if __name__ == "__main__":
    ...
    if args.verbose:
        ...
        credentials='''    sudo echo "{}" |\n  sudo openconnect --protocol=gp --user="{}" --usergroup={}:{} --passwd-on-stdin "{}"\n'''.format(
            cv, un, ('portal' if args.portal else 'gateway'), cn, args.server)
        print(credentials, file=stderr)
        resultFile=open('credentials', 'w')
        resultFile.write(credentials)
        resultFile.close()
